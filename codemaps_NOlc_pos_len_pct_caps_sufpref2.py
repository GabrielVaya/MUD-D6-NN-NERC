...
class Codemaps :
    ...
    def __init__(self, data, maxlen=None, suflen=None, preflen=None, suflen2=None, preflen2=None) :

        if isinstance(data,Dataset) and maxlen is not None and suflen is not None and preflen is not None and suflen2 is not None and preflen2 is not None:
            self.__create_indexs(data, maxlen, suflen, preflen, suflen2, preflen2)

        elif type(data) == str and maxlen is None and suflen is None and preflen is None and suflen2 is None and preflen2 is None:
            self.__load(data)
    ...
    def __create_indexs(self, data, maxlen, suflen, preflen, suflen2, preflen2) :

        self.maxlen = maxlen
        self.suflen = suflen
        self.preflen = preflen
        self.suflen2 = suflen2
        self.preflen2 = preflen2
        words = set([])
        sufs = set([])
        prefs = set([])
        sufs2 = set([])
        ...
        for s in data.sentences() :
            ...
            for i,t in enumerate(s) :
                ...
                prefs2.add(t['lc_form'][-self.preflen2:])
                labels.add(t['tag'])
                pos_tags.add(pos_tagged[i][1])
                w_len.add(len(t['form']))
                if any(char.isupper() for char in t['form']):
                    has_caps.add(True)
                else:
                    has_caps.add(False)
                if any(char in string.punctuation for char in t['form']):
                    has_punct.add(True)
                else:
                    has_punct.add(False)
        ...
        self.suf2_index = {s2: i+2 for i,s2 in enumerate(list(sufs2))}
        self.suf2_index['PAD'] = 0  # Padding
        self.suf2_index['UNK'] = 1  # Unknown suffixes

        self.pref2_index = {p2: i+2 for i,p2 in enumerate(list(prefs2))}
        self.pref2_index['PAD'] = 0  # Padding
        self.pref2_index['UNK'] = 1  # Unknown prefixes
        ...
        self.punct_index = {punct: i+2 for i,punct in enumerate(list(has_punct))}
        self.punct_index['PAD'] = 0  # Padding
        self.punct_index['UNK'] = 1  # Unknown pos

        self.caps_index = {cap: i+2 for i,cap in enumerate(list(has_caps))}
        self.caps_index['PAD'] = 0  # Padding
        self.caps_index['UNK'] = 1  # Unknown pos
    ...
    def __load(self, name) : 
        ...
        with open(name+".idx") as f :
            for line in f.readlines(): 
                ...
                elif t == 'SUF2': self.suf2_index[k] = int(i)
                elif t == 'PREF2': self.pref2_index[k] = int(i)
                elif t == 'POS': self.pos_index[k] = int(i)
                elif t == 'LEN': self.len_index[k] = int(i)
                elif t == 'PUNCT': self.punct_index[k] = int(i)
                elif t == 'CAP': self.caps_index[k] = int(i)
    ...
    def save(self, name) :
        ...
        with open(name+".idx","w") as f :
            ...
            print ('PREFLEN2', self.preflen2, "-", file=f)
            ...
            for key in self.suf2_index : print('SUF2', key, self.suf2_index[key], file=f)
            for key in self.pref2_index : print('PREF2', key, self.pref2_index[key], file=f)
            ...
            for key in self.punct_index : print('PUNCT', key, self.punct_index[key], file=f)
            for key in self.punct_index : print('CAP', key, self.caps_index[key], file=f)
    ...
    def encode_words(self, data) :        
        ...
        # encode and pad suffixes
        Xs2 = [[self.suf2_index[w['lc_form'][-self.suflen2:]] if w['lc_form'][-self.suflen2:] in self.suf2_index else self.suf2_index['UNK'] for w in s] for s in data.sentences()]
        Xs2 = pad_sequences(maxlen=self.maxlen, sequences=Xs2, padding="post", value=self.suf2_index['PAD'])
        # encode and pad prefixes
        Xp2 = [[self.pref2_index[w['lc_form'][-self.preflen2:]] if w['lc_form'][-self.preflen2:] in self.pref2_index else self.pref2_index['UNK'] for w in s] for s in data.sentences()]
        Xp2 = pad_sequences(maxlen=self.maxlen, sequences=Xp2, padding="post", value=self.pref2_index['PAD'])
        ...
        # encode and pad punct
        Xpunct = [[1 if any(char in string.punctuation for char in w['form']) else 0 for w in s] for s in data.sentences()]
        Xpunct = pad_sequences(maxlen=self.maxlen, sequences=Xpunct, padding="post", value=0)
        # Encode and pad caps
        Xcaps = [[1 if any(char.isupper() for char in w['form']) else 0 for w in s] for s in data.sentences()]
        Xcaps = pad_sequences(maxlen=self.maxlen, sequences=Xcaps, padding="post", value=0)
        # return encoded sequences
        return [Xw,Xs,Xp,Xs2,Xp2,Xpos,Xlen,Xpunct,Xcaps]
    ...
    def get_n_sufs2(self) :
        return len(self.suf2_index)
    ## -------- get pref index size ---------
    def get_n_prefs2(self) :
        return len(self.pref2_index)
    ## -------- get pos index size ---------
    def get_n_pos(self) :
    ...
    ## -------- get pos index size ---------
    def get_n_punct(self) :
        return len(self.punct_index)
    ## -------- get pos index size ---------
    def get_n_cap(self) :
    ...
    ## -------- get index for given suffix --------
    def suff22idx(self, s) :
        return self.suf2_index[s]
    ## -------- get index for given preffix --------
    def pref22idx(self, s) :
        return self.pref2_index[s]
    ...
    ## -------- get index for given len-------
    def pct2idx(self, s) :
        return self.punct_index[s]
    ## -------- get index for given len-------
    def cap2idx(self, s) :
        return self.caps_index[s]
